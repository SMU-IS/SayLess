<template>
  <div>
    <div v-if="chatList.length === 0" class="no-chats text-center">
      <p class="text-white">No chats to display.</p>
    </div>
    <div v-else>
      <div v-for="chat in chatList" :key="chat.id" class="chat-item">
        <router-link
          :to="{ name: 'chat-details', params: { chatId: chat.id } }"
        >
          <div
            class="chatContent flex items-center p-2 border-b border-gray-300"
          >
            <img
              :src="chat.profilePic"
              alt="Profile Pic"
...
<script>
import { chatList } from '@/data/chatList.js';
